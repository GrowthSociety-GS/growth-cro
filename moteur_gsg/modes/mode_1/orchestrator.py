...
def run_mode_1_persona_narrator(
    ...
    *,
    ...
    if verbose:
        logger.info(f"  Router racine : {ctx.completeness_pct}% completeness ({len(ctx.available_artefacts)} artefacts)")
        logger.info(f"    Available : {', '.join(ctx.available_artefacts[:8])}")
        if ctx.missing_artefacts:
            logger.info(f"    Missing   : {', '.join(ctx.missing_artefacts[:6])}{' ...' if len(ctx.missing_artefacts) > 6 else ''}")
    ...
    if verbose:
        ...
        if sys_chars > 8192:
            logger.info(f"  ⚠️  ANTI-PATTERN #1 ALERT : system {sys_chars} > 8K (régression -28pts)")
        logger.info(f"  Founder persona : {'hardcoded ⭐' if founder_used else 'extracted from brand_dna'}")
        if philosophy_refs:
            ref_names = [f"{r['site']}/{r['page']}" for r in philosophy_refs[:3]]
            logger.info(f"  Golden refs (AD-3) : {len(philosophy_refs)} matches — top : {ref_names}")
    ...
    if verbose:
        if vision_images:
            logger.info(f"  Vision input : {len(vision_images)} screenshots — {[p.name for p in vision_images]}")
        else:
            logger.info("  Vision input : ❌ aucun screenshot disponible (Sonnet code à l'aveugle)")
    ...
    if apply_post_gates:
        ...
        if verbose:
            if repairs_applied:
                logger.info(f"  🔧 AUTO-REPAIR fonts: {len(repairs_applied)} substitution(s)")
                for r in repairs_applied[:5]:
                    logger.info(f"     - {r}")
            else:
                logger.info("  ✓ AUTO-REPAIR fonts : 0 substitution nécessaire")
    ...
    if apply_post_gates:
        ...
        if visual_slop_violations:
            if verbose:
                logger.info(f"  ⚠️ AI-slop visual patterns détectés : {len(visual_slop_violations)}")
                for v in visual_slop_violations[:5]:
                    logger.info(f"     - {v['pattern']} (severity={v['severity']}, count={v['count']})")
            if repair_visual_slop:
                ...
                if verbose and visual_slop_repairs:
                    logger.info(f"  🔧 Auto-repaired : {len(visual_slop_repairs)} pattern(s)")
                ...
                if verbose and remaining:
                    logger.info(f"  ⚠️ Patterns restants après repair (structurels, à refondre) : {len(remaining)}")
                    for v in remaining[:3]:
                        logger.info(f"     - {v['pattern']} : {v['fix']}")
            elif verbose:
                logger.info("  ↳ report-only (repair_visual_slop=True pour l'ancien repair agressif)")
        else:
            if verbose:
                logger.info("  ✓ AI-slop visual patterns : 0 détection")
    ...
    if apply_post_gates:
        ...
        if verbose:
            if post_gate_violations["aura_font"]:
                logger.info(f"  ⚠️ AURA font blacklist VIOLATIONS post-repair : {post_gate_violations['aura_font']}")
            else:
                logger.info("  ✓ AURA font blacklist : OK (post-repair)")
            if post_gate_violations["design_grammar"]:
                logger.info(f"  ⚠️ design_grammar VIOLATIONS : {post_gate_violations['design_grammar']}")
            else:
                logger.info("  ✓ design_grammar forbidden patterns : OK")
    ...
    if verbose:
        ...
        logger.info(f"  HTML chars      : {len(html_fixed)}")
        if audit:
            t = audit.get("totals", {})
            logger.info(f"  Doctrine score  : {t.get('total_pct','?')}% (info only — pas blocking)")
