"""Google Ads audit CLI (axis #5).

Usage:
    python -m growthcro.audit_gads.cli --client <slug> --csv <path> [--out-dir <path>]
                                       [--period <label>] [--business-category <cat>]
                                       [--notes <text>] [--no-write] [--json]

Persists three artefacts under `data/audits/gads/<client>/<period>/`:
...
from growthcro.audit_gads.orchestrator import AuditInputs, run_audit
...
def _default_out_dir(client_slug: str, period: str) -> pathlib.Path:
    return _project_root() / "data" / "audits" / "gads" / client_slug / period
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="growthcro.audit_gads.cli",
        description="Run a Google Ads audit via the gads-auditor skill (thin wrapper).",
    )
    ...
    parser.add_argument(
        ...
        required=True,
        type=pathlib.Path,
        help="Path to Google Ads Editor / Reports CSV export.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not csv_path.exists():
        print(f"[audit_gads] CSV not found: {csv_path}", file=sys.stderr)
        return 2
    ...
    if args.no_write:
        if not args.json:
            print(
                f"[audit_gads] preview ok — client={args.client} period={inputs.normalised_period()}"
    ...
    print(f"[audit_gads] artefacts written → {out_dir}")
