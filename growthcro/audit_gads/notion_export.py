"""Notion-template renderer for Google Ads audits (axis #8 — I/O serialization).
...
The Growth Society template canonicalises 8 sections (A–H). Keeping the renderer
deterministic means we can validate the layout in tests without invoking the
skill.
"""
...
def _kpi_lines(kpis: dict) -> list[str]:
    ...
    for key in order:
        ...
        elif key in {"cost", "conv_value", "avg_cpc", "cpa"}:
            lines.append(f"- **{key}**: {value}")
        else:
            lines.append(f"- **{key}**: {value}")
    return lines
...
def _campaigns_lines(section_b: dict) -> list[str]:
    lines: list[str] = []
    breakdown = section_b.get("breakdown_by_type") or []
    if breakdown:
        lines.append("**Répartition par type:**")
        lines.append("")
        lines.append("| Type | Campagnes | Cost | Conversions | ROAS |")
        lines.append("|---|---:|---:|---:|---:|")
        for row in breakdown:
            lines.append(
                "| {ctype} | {camps} | {cost} | {conv} | {roas} |".format(
                    ctype=row.get("campaign_type", "-"),
                    camps=row.get("campaigns", 0),
                    cost=row.get("cost", 0.0),
                    conv=row.get("conversions", 0.0),
    ...
    if top:
        ...
        for camp in top:
            lines.append(
                "- {name} ({ctype}) — cost={cost}, conv={conv}, roas={roas}".format(
                    name=camp.get("campaign", "-"),
                    ctype=camp.get("campaign_type", "-"),
                    cost=camp.get("cost", 0.0),
                    conv=camp.get("conversions", 0.0),
...
def render_notion_markdown(bundle: dict) -> str:
    """Render a Markdown page conforming to the Growth Society template.

    `bundle` is `AuditBundle.as_dict()` (or equivalent). The output is safe to
    paste into Notion via "Import from Markdown" or to attach as a fenced block.
    """
    sections = bundle.get("sections", {})
    title = (
        f"# Audit Google Ads — {bundle.get('client_slug', 'client')}"
    ...
    parts: list[str] = [
        title,
        "",
        f"**Platform**: {bundle.get('platform', 'google_ads')}",
        f"**Skill**: {bundle.get('skill_name', 'anthropic-skills:gads-auditor')}",
...
def render_notion_payload(bundle: dict) -> dict:
    """Render a Notion-API-compatible page payload (properties + children blocks).

    Caller wires the actual `parent` (database_id or page_id) and passes this
    payload to `notion.pages.create`. We keep the renderer headless so it can
    be tested + cached.
    """
    sections = bundle.get("sections", {})
    title_text = (
    ...
    return {
        ...
        "metadata": {
            "platform": bundle.get("platform", "google_ads"),
            "skill_name": bundle.get("skill_name", "anthropic-skills:gads-auditor"),
...
def render_bundle_json(bundle: dict, *, indent: int = 2) -> str:
    """Pretty-print the bundle as JSON (useful for `--json` CLI output)."""
